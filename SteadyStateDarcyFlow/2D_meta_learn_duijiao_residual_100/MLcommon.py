...
from core.linear_eq_solver import SuperLU_GPU, spsolve_lu
from core.model import Domain2D

from common import EquSolver
...
class GaussianElliptic2Torch(GaussianElliptic2):
    ...
    ## When faced the sparse matrix, the autograd mechnism seems cannot work well
    # def evaluate_CM_inner(self, u_vec, v_vec=None):
    #     if v_vec is None:
    #         v_vec = u_vec
            
    #     if self.is_torch == True:
    #         ## the backward cannot be correctly work
    #         if type(u_vec) != type(self.mean_vec_torch):
    #             u_vec = torch.tensor(u_vec, dtype=self.mean_vec_torch.dtype)
    #         if type(v_vec) != type(self.mean_vec_torch):
    #             v_vec = torch.tensor(v_vec, dtype=self.mean_vec_torch.dtype)
    #         temp1 = u_vec - self.mean_vec_torch 
    #         temp2 = v_vec - self.mean_vec_torch 
    #         if temp1.ndim == 1:
    #             temp1 = temp1.reshape(-1,1).contiguous()
    #             out = torch.sparse.mm(self.K_torch, temp1)
    #             out = self.Minv_torch*out
    #             out = torch.sparse.mm(self.KT_torch, out)
    #             # out = torch.mv(self.K_torch, out)
    #             out = torch.sum(temp2*out)
    #         else:
    #             out = self.Minv_torch*torch.sparse.mm(self.K_torch, temp1.T)
    #             out = torch.sparse.mm(self.KT_torch, out) 
    #             out = torch.sparse.mm(temp2.T, out)
    #             out = out.diagonal()
    #     elif self.is_torch == False:
    #         u_vec = np.array(u_vec, dtype=self.mean_vec.dtype)
    #         v_vec = np.array(v_vec, dtype=self.mean_vec.dtype)
    #         temp1 = u_vec - self.mean_vec
    #         temp2 = v_vec - self.mean_vec
    #         if temp1.ndim == 1:
    #             out = spsl.spsolve(self.M, self.K@temp1)
    #             # out = 1.0/self.M.diagonal()*(self.K@temp1)
    #             out = self.K.T@out
    #             out = temp2@out
    #         else:
    #             out = spsl.spsolve(self.M, self.K@temp1.T)
    #             out = self.K.T@out
    #             out = temp2.T@out
    #             out = out.diagonal()
...
# class GaussianElliptic2TorchAdaptive(GaussianElliptic2Torch):
#     def __init__(self, domain, alpha=1.0, a_fun=fe.Constant(1.0), theta=1.0, 
#                  mean_fun=None, tensor=False, boundary='Neumann', invM='full', 
#                  use_LU=True, num_adaptive_lam=None):
#         super().__init__(domain=domain, alpha=alpha, a_fun=a_fun, theta=theta, 
#                      mean_fun=mean_fun, tensor=tensor, boundary=boundary, invM=invM, 
#                      use_LU=use_LU)
#         self.num_adaptive_lam = num_adaptive_lam
#         self.is_eig_available = False    
        
#     def calculate_eigensystem(self, num_redudent=20):
...
#         assert self.num_adaptive_num != None
#         assert self.num_adaptive_lam <= len(self.mean_vec) - num_redudent
...
class PDEFun(torch.autograd.Function):
    ...
    def forward(ctx, input, equ_solver):
        ...
        if device_input.type == "cpu": 
            ...
            # S = torch.tensor(equ_solver.S, dtype=dtype)
            # lu = spsl.splu(equ_solver.K.tocsc())
            # sol = lu.solve(equ_solver.F)
            # output = torch.tensor((equ_solver.S@sol).T, dtype=dtype).squeeze()
            # lu_U = spnumpy2sptorch(lu.U, dtype=dtype)
            # lu_L = spnumpy2sptorch(lu.L, dtype=dtype)
            # lu_perm_c = torch.tensor(lu.perm_c, dtype=dtype) 
            # lu_perm_r = torch.tensor(lu.perm_r, dtype=dtype)
            # ctx.save_for_backward(
            #     input, S, lu_U, lu_L, lu_perm_c, lu_perm_r
            #     )
            ...
            # sol = lu.solve(equ_solver.F)
            # output = torch.tensor((equ_solver.S@sol).T, dtype=dtype).squeeze()
            # lu_shape = torch.tensor(lu.shape) 
            # lu_nnz = torch.tensor(lu.nnz)
            # lu_U = spnumpy2sptorch(lu.U, dtype=dtype)
            # lu_L = spnumpy2sptorch(lu.L, dtype=dtype)
            # lu_perm_c = torch.tensor(lu.perm_c, dtype=dtype) 
            # lu_perm_r = torch.tensor(lu.perm_r, dtype=dtype)
            # ctx.save_for_backward(
            #     input, S, lu_shape, lu_nnz, lu_U, lu_L, lu_perm_c, lu_perm_r
            #     )
        elif device_input.type == "cuda":  
            pass
            # # a = time.time()
            # K_gpu = cpss.csr_matrix(equ_solver.K)
            # F_gpu = cp.asarray(equ_solver.F)
            # S_gpu = cp.asarray(equ_solver.S)
            # KK = spnumpy2sptorch(equ_solver.K, dtype=dtype)
            # SS = torch.tensor(equ_solver.S, dtype=dtype, device=device_input)
            # if type(equ_solver.init_adjoint_sol) == type(None):
            #     ctx.save_for_backward(input, KK, SS)
            # else:
            #     init_adjoint_sol = from_dlpack(equ_solver.init_adjoint_sol.toDlpack())
            #     init_adjoint_sol = init_adjoint_sol.type(dtype)
            #     ctx.save_for_backward(input, KK, SS, init_adjoint_sol)
            # # a = time.time()
            # if type(equ_solver.init_forward_sol) == type(None):
            #     sol = cpssl.lsqr(K_gpu, F_gpu)
            # else:
            #     sol = cpssl.gmres(
            #         K_gpu, F_gpu, x0=equ_solver.init_forward_sol, tol=1e-3
            #         )[0]
            # # equ_solver.init_forward_sol = sol
            # output = S_gpu@sol
            # output = from_dlpack(output.toDlpack())
    ...
    def backward(ctx, grad_output):
        ...
        if device.type == "cpu":
            ...
            # input, S, lu_U, lu_L, lu_perm_c, lu_perm_r = ctx.saved_tensors
            # S = S.numpy()
            # grad_output_numpy = np.array(grad_output.cpu())
            # Fs = -S.T@(grad_output_numpy.T)
            # lu_U, lu_L = sptorch2spnumpy(lu_U), sptorch2spnumpy(lu_L)
            # lu_perm_c = np.array(lu_perm_c).astype(np.int32)
            # lu_perm_r = np.array(lu_perm_r).astype(np.int32)
            # sol = spsolve_lu(
            #     L=lu_L, U=lu_U, b=Fs, perm_c=lu_perm_c, perm_r=lu_perm_c
            #     )
            # val = torch.tensor(sol, dtype=dtype).squeeze()
            
            # input, S, lu_shape, lu_nnz, lu_U, lu_L, lu_perm_c, lu_perm_r = ctx.saved_tensors 
            # S = S.numpy()
            # grad_output_numpy = np.array(grad_output.cpu())
            # Fs = -S.T@(grad_output_numpy.T)
            # lu_shape = lu_shape.numpy().astype(np.int32)
            # lu_nnz = lu_nnz.numpy().astype(np.int32)
            # lu_U, lu_L = sptorch2spnumpy(lu_U), sptorch2spnumpy(lu_L)
            # lu_perm_c = np.array(lu_perm_c).astype(np.int32)
            # lu_perm_r = np.array(lu_perm_r).astype(np.int32)
            # lu_gpu = SuperLU_GPU(
            #     shape=lu_shape, L=lu_L, U=lu_U, perm_r=lu_perm_r, perm_c=lu_perm_c,
            #     nnz=lu_nnz,
            #     )
            # sol = lu_gpu.solve(cp.array(Fs))
            # sol = from_dlpack(sol.toDlpack())
            # sol = sol.type(dtype)
            # val = sol.cpu().squeeze()
        elif device.type == "cuda":
            pass
            # input, K, S, init_adjoint_sol = ctx.saved_tensors
            # # a = time.time()
            # K = sptorch2spnumpy(K)
            # K_gpu = cpss.csr_matrix(K)
            # # S = cp.asarray(S)
            # S = to_dlpack(S)
            # S = cp.from_dlpack(S)
            # # b = time.time()
            # # print(b-a)
            # ## convert grad_output to cupy array
            # cp_go = to_dlpack(grad_output)
            # cp_go = cp.from_dlpack(cp_go)
            # Fs_gpu = cp.matmul(-S.T, cp_go.T)
            # # b = time.time()
            # # print(b-a)
            # # S = S.numpy()
            # # grad_output_numpy = np.array(grad_output)
            # # Fs = -S.T@(grad_output_numpy.T) 
            # # Fs = torch.matmul(-S.T, grad_output.T)
            # # # convert to cupy array
            # # Fs = to_dlpack(Fs)
            # # Fs_gpu = cp.from_dlpack(Fs)
            # # a = time.time()
            # if type(init_adjoint_sol) == type(None):
            #     sol = cpssl.lsqr(K_gpu, Fs_gpu)
            # else:
            #     init_adjoint_sol = to_dlpack(init_adjoint_sol)
            #     init_adjoint_sol = cp.from_dlpack(init_adjoint_sol)
            #     sol = cpssl.gmres(
            #         K_gpu, Fs_gpu, x0=init_adjoint_sol, tol=1e-3
            #         )[0]
            # # sol = cpssl.lsqr(K_gpu, Fs_gpu)[0]
            # # sol = cpssl.spsolve(K_gpu, Fs_gpu)
            # val = from_dlpack(sol.toDlpack())
            # val = val.type(dtype)
            # # b = time.time()
            # # print(b-a)
        else:
...
class Dis2Fun(object):
    ...
    def dis2fun(self, d):
        # Sy = np.sqrt(1.0/equ_solver.M.diagonal())*((equ_solver.S.T)@d)
        # Sy = 1.0/equ_solver.M.diagonal()*((equ_solver.S.T)@d)
        # Sy = spsl.spsolve(equ_solver.M, (equ_solver.S.T)@d)
        F = ((self.S.T)@d).reshape(-1,1)
        Sy = self.lu.solve(F)
        # Sy = spsl.spsolve(self.M + alpha*self.A, F)
