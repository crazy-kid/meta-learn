...
# -*- coding: utf-8 -*-
"""
Created on Tue Jul 19 20:00:51 2022
...
# dir: str = os.path.abspath(os.path.join(dir, "../.."))  # 获得项目的根目录
# sys.path.append(dir)

from core.probability import GaussianElliptic2
from core.eigensystem import double_pass  # 没被使用过
...
# from torch_sparse_solve import solve as torchspsolve  # ?这个文件呢 为什么import不了 这个文件没有被使用
...
class GaussianElliptic2Learn(GaussianElliptic2):
    def __init__(self, domain, alpha=1.0, a_fun=fe.Constant(1.0), theta=1.0,
        ...
        aa1 = fe.Constant(self._alpha) * fe.inner(self._theta *
                                                  fe.grad(u), fe.grad(v)) * fe.dx
    ...
    def trans2learnable(self, learn_mean=True, learn_var=True):
        self.mean_vec_learn = torch.tensor(
            self.mean_fun.vector()[
                :], dtype=torch.float64, requires_grad=learn_mean
        )
        ...
        self.log_gamma_learn = torch.tensor(
            0.0, dtype=torch.float64, requires_grad=learn_var)
        ...
        self.M_lamped_half_torch = spnumpy2sptorch(
            self.M_lamped_half, dtype=torch.float64)
    ...
    def generate_sample_zero_mean(self, gamma=1.0, method='numpy'):
        ...
        # t1 = gamma*(self.K2_torch.to_sparse_coo())
        # KK = self.K1_torch.to_sparse_coo() + t1
        # KK3 = torch.stack([KK for _ in range(1)], dim=0)
        # n = np.random.normal(0, 1, (self.function_space_dim,))
        # n = torch.tensor(n, dtype=torch.float64)
        # b = self.M_lamped_half_torch@n
        # b = b[None,:,None]
        # fun_vec = torchspsolve(KK3, b)[0,:,0]
        # return fun_vec
        ...
        # KK = self.K1 + gamma*self.K2
        # if method == 'numpy':
        #     n = np.random.normal(0, 1, (self.function_space_dim,))
        #     b = self.M_lamped_half@n
        #     self.boundary_vec(b)
        #     if self.use_LU == False:
        #         fun_vec = spsl.spsolve(KK, b)
        #     elif self.use_LU == True:
        #         raise NotImplementedError("use_LU must be False")
        #     else:
        #         raise NotImplementedError("use_LU must be True or False")
        #     return np.array(fun_vec)
        # elif method == 'FEniCS':
        #     raise NotImplementedError("method must be FEniCS")
        # else:
    ...
    def generate_sample_learn(self, method='numpy'):
        ...
        # sample_ = torch.tensor(sample_, dtype=torch.float64)
        sample = self.mean_vec_learn + sample_
        # if self.bc == "Dirichlet":
        #     # In order to add Dirichlet boundary condition, we can think of a problem
        #     # Id(u) = f, with Id is the identity operator with Dirichlet boundary condition.
        #     # Through FEM discretization, Mu = Mf => u = M^{-1}Mf. Since the Dirichlet condition
        #     # we actually need to apply bc.apply(Mf) before we apply M^{-1} to Mf.
        #     # Here, for simplicity, we calculate M^{-1} only by its diagonal elements.
        #     sample = torch.mv(self.M_torch, sample)
        #     sample[self.index_boundary] = 0.0
        #     sample = self.Minv_torch*sample
        return sample

    # def evaluate_CM_inner_torch(self, u_vec, v_vec=None):
    #     if v_vec is None:
    #         v_vec = u_vec
    #     temp1 = u_vec - self.mean_vec_learn
    #     temp2 = v_vec - self.mean_vec_learn
    #     out = self.Minv_torch*torch.mv(self.K_torch, temp1)
    #     out = torch.mv(self.K_torch, out)
...
class GaussianFiniteRank(object):
    ...
    def __init__(self, domain, domain_=None, mean=None, num_gamma=50, num_KL=None,
        ...
        u_, v_ = fe.TrialFunction(
            domain.function_space), fe.TestFunction(domain.function_space)
        ...
        u_, v_ = fe.TrialFunction(domain_.function_space), fe.TestFunction(
            domain_.function_space)
        ...
        if mean is None:
            ...
            self.mean_vec = fe.interpolate(
                mean, self.domain.function_space).vector()[:]
        ...
        # self.f2sM = construct_measurement_matrix(coor[v2d], self.domain.function_space)

        # coor = self.domain.mesh.coordinates()
        # v2d = fe.vertex_to_dof_map(self.domain.function_space)
        # ## small to full matrix
        # self.s2fM = construct_measurement_matrix(coor[v2d], self.domain_.function_space)
        ...
        # full to small matrix
        self.f2sM = construct_measurement_matrix(
            coor[d2v], self.domain.function_space)

        coor = self.domain.mesh.coordinates()
        # v2d = fe.vertex_to_dof_map(self.domain.function_space)
        d2v = fe.dof_to_vertex_map(self.domain.function_space)
        # small to full matrix
        self.s2fM = construct_measurement_matrix(
            coor[d2v], self.domain_.function_space)
    ...
    def _K_org_x_op(self):
        linear_op = spsl.LinearOperator(
            (self.dim, self.dim), matvec=self._K_org_x)
        return linear_op
    ...
    def _Minv_x_op(self):
        linear_op = spsl.LinearOperator(
            (self.dim, self.dim), matvec=self._Minv_x)
        return linear_op
    ...
    def calculate_eigensystem(self, l=20):
        ...
        # if self.num_KL  + l > self.dim:
        #     l = self.dim - self.num_KL
        # omega = np.random.randn(self.dim, self.num_KL + l)

        # The following two methods are may be mathematically equivalent.
        # However, the second one will be numerically much more stable.
        # The largest eigenvalue lam corresponding to the smallest eigenvalue
        # \sigma, so we may need to set self.num_KL = self.dim to generate
        # accurate approximations.
        # This will be computationally expensive for large-scale problems.
        # Compute the eigensystem for K_op^{-1} seems unstable! Why ??

        # self.sigma_inv, self.eigvec = double_pass(
        #     self._K_org_inv_x, M=self._Minv_x, Minv=self._M_x, r=self.num_KL, Mg=self.Ms, l=l,
        #     omega=omega, cutval=0.0
        #     )
        # self.lam = np.power(self.sigma_inv, self.s/2)
        # self.eigvec = spsl.spsolve(self.M, self.eigvec)

        # self.sigma, self.eigvec_ = double_pass(
        #     self._K_org_x, M=self._M_x, Minv=self._Minv_x, r=self.num_KL, Mg=self.Ms, l=l,
        #     omega=omega, cutval=0.0
        #     )
        # self.eigvec_ = np.flip(self.eigvec_, axis=1)
        # self.sigma = np.flip(self.sigma)
        if self.boundary == "NeumannZero":
            self.sigma, self.eigvec_ = sl.eigh(
                self.K_org.todense(), self.Ms.todense())
        ...
        # transfer results from rough grid to fine grid
        # This seems to be the a time consuming process by the following implementation!
        # Relying on rewrite interpolate, this process can be accelerated.
        # However, it is not important for our purpose now.
        # I may satisfied with this implementation at this moment.
        # start = time.time()
        # self.eigvec = self.eigvec_
        # self.eigvec = np.zeros((self.dim_full, self.dim))
        # for itr in range(self.dim):
            # self.fun_.vector()[:] = np.array(self.eigvec_[:,itr])
            # self.eigvec[:, itr] = fe.interpolate(self.fun_, self.domain.function_space).vector()[:]
        self.eigvec = self.s2fM @ self.eigvec_
        # end = time.time()
        # print(end - start)

        # Using the functions in scipy, similar behaviors as using the double_pass
        # implemented by ourself.
        # _K_org_x_op = self._K_org_x_op()
        # _K_org_inv_x_op = self._K_org_inv_x_op()
        # _M_x_op = self._M_x_op()
        # _Minv_x_op = self._Minv_x_op()
        # if self.num_KL == self.dim: self.num_KL = self.num_KL - 10
        # self.sigma_inv, self.eigvec = spsl.eigsh(
        #         _K_org_inv_x_op, M=_Minv_x_op, k=self.num_KL, which='LM',
        #         Minv=_M_x_op
        #         )
        # # self.sigm = np.flip(self.sigma)
        # # self.eigvec = np.flip(self.eigvec, axis=1)
        # # self.lam = np.power(self.sigma, -self.s/2)
        # self.lam = np.power(self.sigma_inv, self.s/2)
        # self.eigvec = spsl.spsolve(self.Ms, self.eigvec)
    ...
    def trans2learnable(self, mean=True, log_gamma=False):
        ...
        if mean == True:
            self.mean_vec_learn = torch.tensor(
                self.mean_vec, dtype=torch.float32, requires_grad=True)
        else:
            self.mean_vec_learn = torch.tensor(
                self.mean_vec, dtype=torch.float32, requires_grad=False)
        ...
        self.lam_res = torch.tensor(
            self.lam_res, dtype=torch.float32, requires_grad=False)
        self.lam = torch.tensor(
            self.lam, dtype=torch.float32, requires_grad=False)
        self.eigvec = torch.tensor(
            self.eigvec, dtype=torch.float32, requires_grad=False)
        self.eigvec_ = torch.tensor(
            self.eigvec_, dtype=torch.float32, requires_grad=False)
        if type(self.Ms) == np.ndarray:
            self.Ms = torch.tensor(
                self.Ms, dtype=torch.float32, requires_grad=False)
        else:
            self.Ms = torch.tensor(
                self.Ms.todense(), dtype=torch.float32, requires_grad=False)
        self.M = spnumpy2sptorch(self.M)
        if type(self.f2sM) == np.ndarray:
            self.f2sM = torch.tensor(
                self.f2sM, dtype=torch.float32, requires_grad=False)
        else:
            self.f2sM = torch.tensor(
                self.f2sM.todense(), dtype=torch.float32, requires_grad=False)
        if type(self.s2fM) == np.ndarray:
            self.s2fM = torch.tensor(
                self.s2fM, dtype=torch.float32, requires_grad=False)
        else:
            self.s2fM = torch.tensor(
                self.s2fM.todense(), dtype=torch.float32, requires_grad=False)
        if self.boundary == "DirichletZero":
            if type(self.Ms0) == np.ndarray:
                self.Ms0 = torch.tensor(
                    self.Ms0, dtype=torch.float32, requires_grad=False)
            else:
                self.Ms0 = torch.tensor(
                    self.Ms0.todense(), dtype=torch.float32, requires_grad=False)
    ...
    def trans2numpy(self):
        if self.is_tensor == True:
            self.mean_vec = np.array(
                self.mean_vec_learn.cpu().detach().numpy())
            self.log_gamma = np.array(
                self.log_gamma_learn.cpu().detach().numpy())
...
class PriorFunFR(torch.autograd.Function):
    ...
    def forward(ctx, input, prior):
        ...
        if prior.num_gamma < prior.num_KL:
            val = torch.cat([
                torch.pow(torch.exp(prior.log_gamma), -2) *
                val[:prior.num_gamma],
