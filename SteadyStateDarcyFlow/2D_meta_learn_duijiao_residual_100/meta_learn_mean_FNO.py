...
# 没有被使用 将"/home/rick/.local/lib/python3.10/site-packages"添加到python.analysis.extraPaths中 因为python是用conda装的 但是cupy是用pip安装的
# pip 安装没有用 要是要用conda 且安装在相应环境中
import cupy as cp
...
for kk in range(k_start, max_iter):
    ...
    if kk % 10 == 0:
        ...
        for it in draw_num:
            ...
            # fig = plt.imshow(draw_truth, vmin=0.0, vmax=3.5)
            fig = plt.imshow(draw_truth)
            plt.colorbar(fig)
            plt.title("Meta u")
            plt.subplot(2, 2, 2)
            # fig = fe.plot(fun)
            # fig = plt.imshow(draw_est, vmin=0.0, vmax=3.5)
