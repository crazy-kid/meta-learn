...
from MLcommon import Dis2Fun, UnitNormalization, GaussianFiniteRankTorch, \
    FNO2d, ForwardProcessNN, LpLoss
...
if env == "complex":
    ...
    kk_f = int(2500)      # env = 'complex'
elif env == "simple":
    kk_FNO = int(3100)   # env = 'simple'
    kk_f = int(6200)     # env = 'simple'
else:
...
n_start, n_end = 0, 50
max_iter_num = 45  # maximum iteration number of the optimization algorithm
...
# train_dataset_Sy = np.load(meta_data_dir + "train_dataset_Sy_" + str(num_points) + ".npy")
...
# test_dataset_Sy = np.load(meta_data_dir + "test_dataset_Sy_" + str(num_points) + ".npy")
...
forward_nn = ForwardProcessNN(
    ...
    # normalize_data=None, normalize_param=None,
    device=device
    )
...
if comput_unlearned == True:
    ...
    for idx in range(n_start, n_end):
        ...
        for itr in range(max_iter):
            newton_cg.descent_direction(cg_max=cg_max, method='cg_my')
            # newton_cg.descent_direction(cg_max=cg_max, method='bicgstab')
            ...
            tmp_list.append(relative_error(fun_unlearn, fun_truth))
        ...
        # unlearned_error_list.append(relative_error(fun_unlearn, fun_truth))
        if len(tmp_list) > 1:
    ...
    unlearned_error_list = np.array(unlearned_error_list)
    ...
    unlearned_error_list = np.load(
        meta_results_dir + "unlearned_error_list.npy", allow_pickle=True
        )
    results_unlearned = np.load(
        meta_results_dir + "estimates_unlearn_MAP.npy", allow_pickle=True
        )
...
for idx in range(n_start, n_end):
    ...
    for itr in range(max_iter):
        ...
        tmp_list.append(relative_error(fun_learned_f, fun_truth))
    ...
    if len(tmp_list) > 1:
        ...
        learned_error_list_f.append([np.array(tmp_list)])
    print("Unlearn Error: ", learned_error_list_f[-1][-1])
...
for idx in range(n_start, n_end):
    ...
    # learned_error_list.append(relative_error(fun_learned, fun_truth))
    learned_error_list.append(np.array(tmp_list))
    if len(learned_error_list[-1]) > 1:
        print("Learned Error: ", learned_error_list[-1][-1])
    else:
    	print("Learned Error: ", learned_error_list[-1])
        
np.save(meta_data_dir + "learned_error_list", np.array(learned_error_list))
np.save(meta_results_dir + "estimates_learn_MAP", (estimates_learn, final_error_learn))
learned_error_list = np.load(meta_data_dir + "learned_error_list.npy", allow_pickle=True)
results_learned = np.load(meta_results_dir + "estimates_learn_MAP.npy", allow_pickle=True)
...
# plt.colorbar(fig)
# plt.subplot(1,3,2)
# param = results_unlearned[0][idx]
# fun_unlearn = fe.Function(domain.function_space)
# fun_unlearn.vector()[:] = np.array(param)
# fig = fe.plot(fun_unlearn)
# plt.title("Estimate (unlearn)")
# plt.colorbar(fig)
# plt.subplot(1,3,3)
# param = results_learned[0][idx]
# fun_learned = fe.Function(domain.function_space)
# fun_learned.vector()[:] = np.array(param)
# fig = fe.plot(fun_learned)
# plt.title("Estimate (learned)")
# plt.colorbar(fig)
# plt.show()

unlearned_error_avg = np.mean(np.array(unlearned_error_list)[:][-1])
learned_error_avg = np.mean(np.array(learned_error_list)[:][-1])
learned_error_avg_f = np.mean(np.array(learned_error_list_f)[:][-1])
