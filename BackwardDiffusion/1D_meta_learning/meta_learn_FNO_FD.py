...
# num_gamma = 60
alpha_prior, beta_prior = 0.01, 0.01
...
#     domain, alpha=0.1, a_fun=fe.Constant(1.0), theta=1.0, 
#     mean_fun=None, tensor=False, boundary='Neumann'
#     )
# prior.trans2learnable()
...
alpha_hyper_prior, beta_hyper_prior = 0.01, 0.01
...
if with_hyper_prior == True:
    ...
    np.save(
        ...
        prior.a_fun_learn.detach().numpy() #prior.log_gamma_learn.detach().numpy()
    )
else:
    torch.save(
        nnprior_mean.state_dict(), meta_results_dir + env +
