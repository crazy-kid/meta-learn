...
class Domain2D(Domain):
    '''
    class Domain has two properties: mesh, function_space
    mesh: a square domain with uniform mesh
    function_space: can be specified by 'mesh_type' and 'mesh_order'

    :param low_point 区域左下角
    :param high_point 区域右上角
    :param nx,ny x轴y轴上的离散数？
    :param _mesh fenics中的矩形区域 私有变量
    :param _function_space fenics中的函数空间
    '''
    ...
    def __init__(self, low_point=[0, 0], high_point=[1, 1], nx=100, ny=100, mesh_type='P', mesh_order=2):
        ...
        self._mesh: fe.mesh = fe.RectangleMesh(fe.Point(low_point[0], low_point[1]),
                                               fe.Point(high_point[0], high_point[1]), nx, ny)  # 没有RectangleMesh
        self._function_space: fe.FunctionSpace = fe.FunctionSpace(
            self._mesh, self.mesh_type, self.mesh_order)
    ...
    def update(self, low_point=[0, 0], high_point=[1, 1], nx=100, ny=100, mesh_type='P', mesh_order=2):
        ...
        self._function_space = fe.FunctionSpace(
            self._mesh, mesh_type, mesh_order)


class Domain1D(Domain):
    def __init__(self, low_point=0, high_point=1, n=100, mesh_type='P', mesh_order=2):
        super().__init__(mesh_type, mesh_order)
        self._mesh: fe.mesh = fe.IntervalMesh(n, low_point, high_point)
        self._function_space = fe.FunctionSpace(
            self._mesh, self.mesh_type, self.mesh_order)
...
class ModelBase(object):
    ...
    def precondition_linear_operator(self):
        leng = self.M.shape[0]
        linear_ope = spsl.LinearOperator(
            (leng, leng), matvec=self.precondition)
        return linear_ope
    ...
    def precondition_inv_linear_operator(self):
        leng = self.M.shape[0]
        linear_ope = spsl.LinearOperator(
            (leng, leng), matvec=self.precondition_inv)
        return linear_ope
