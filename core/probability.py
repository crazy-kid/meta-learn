...
class GaussianElliptic2(object):
    ...
    def __init__(self, domain: Domain, alpha: float = 1.0, a_fun=fe.Constant(1.0), theta: float = 1.0,
                 mean_fun=None, tensor: bool = False, boundary: str = 'Neumann', bc_val=0.0,
                 use_LU=True):
        """
        ...
        self._tensor: bool = tensor

        if self._tensor == False:
            assert type(theta) == type(1.0) or type(theta) == type(
                np.array(1.0)) or type(theta) == type(1)
            self._theta = fe.interpolate(
                fe.Constant(theta), self.domain.function_space)
        elif self._tensor == True:  # theta是向量
        ...
        # 设置均值
        if mean_fun is None:
            self.mean_fun = fe.interpolate(fe.Expression(
                "0.0", degree=2), self.domain.function_space)
        else:
            self.mean_fun = fe.interpolate(
                mean_fun, self.domain.function_space)
    ...
    @alpha.setter
    def alpha(self, al):
        assert type(al) == type(1.0) or type(al) == type(
            np.array(1.0)) or type(al) == type(1)
        self._alpha = al
        self.generate_K()
    ...
    def theta(self, th):
        if self._tensor == False:
            assert type(th) == type(1.0) or type(th) == type(
                np.array(1.0)) or type(th) == type(1)
            self._theta = th
    ...
    def boundary(self, b):
        if self.bc == 'Dirichlet':
            ...
            bc = fe.DirichletBC(self.domain.function_space,
                                fe.Constant(self.bc_val), boundary)
            bc.apply(b)
    ...
    def evaluate_CM_inner(self, u_vec, v_vec, method='numpy'):
        ...
        if method == 'numpy':
            ...
            fe.solve(self.M_, self.temp1.vector(),
                     (self.K_ * self.temp0.vector()))
    ...
    def pointwise_variance_field(self, xx, yy, method="numpy"):
        ...
        SN = construct_measurement_matrix(
            np.array(xx), self.domain.function_space)
        SM = construct_measurement_matrix(
            np.array(xx), self.domain.function_space)
...
class GaussianFiniteRank(object):
    ...
    def generate_sample(self, num_sample=1):
        ...
        if num_sample == 1:
            val = self.mean_vec + \
                self.generate_sample_zero_mean(num_sample=num_sample)
        else:
            val = self.mean_vec.reshape(-1, 1) + \
                self.generate_sample_zero_mean(num_sample=num_sample)
