...
# -*- coding: utf-8 -*-
"""
Created on Fri Aug 19 14:20:19 2022
...
## grid_coordiantes contains coordinates that are match the function values 
## obtained by fun.vector()[:]
...
grid_coordinates = domain.mesh.coordinates()[d2v]
...
for kk in range(max_iter):
    ...
    for itr in batch:
        ...
        ## M^{-1/2}S^{T}d
        dis2fun.reset_points(points)
        Sy = dis2fun(dataset_y[itr])[v2d].reshape(equ_nx+1, equ_nx+1)
        Sy = (Sy.reshape(1, equ_nx+1, equ_nx+1, 1)).to(torch.device(device))
        prior.mean_vec_torch = (nnprior_mean(Sy).reshape(-1))[d2v]
