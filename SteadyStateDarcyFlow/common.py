...
class EquSolver(object):
    ...
    def incremental_forward_solver(self, m_hat, sol_forward=None, method='numpy'):
        ...
        if type(m_hat) == np.ndarray:
            if method == 'FEniCS':
                ...
                b_ = -fe.assemble(fe.inner(self.m_hat * self.exp_m *
                                  fe.grad(self.sol_forward), fe.grad(self.v_)) * fe.dx)
                self.bc.apply(b_)
    ...
    def incremental_adjoint_solver(self, vec, m_hat, sol_adjoint=None, simple=False, method='numpy'):
        ...
        if simple == False:
            if method == 'FEniCS':
                self.m_hat.vector()[:] = np.array(m_hat)
                bl_ = fe.assemble(fe.inner(self.m_hat * self.exp_m *
                                  fe.grad(self.sol_adjoint), fe.grad(self.v_)) * fe.dx)
                self.Fs.vector()[:] = Fs
